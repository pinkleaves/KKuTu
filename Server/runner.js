...
exports.MAIN_MENU = [
	{
		...
		submenu: [
			...
			{
				label: LANG['menu-server-web'],
				//accelerator: "Cmd",
				click: () => exports.run("server-web")
			}
		]
	},
	{
...
function websv(){ //분홍꽃 수정 (204~212)
	if(webServer) webServer.kill();
	if(SETTINGS['server-name']) process.env['KKT_SV_NAME'] = SETTINGS['server-name'];
	webServer = new ChildProcess('W', "node", `${__dirname}/lib/Web/cluster.js`, SETTINGS.web.cpu);
}
function stopServer(){
	if(webServer) webServer.kill();
