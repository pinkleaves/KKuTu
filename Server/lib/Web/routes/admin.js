...
var KOEN = require("../lang/ko_KR.json");
...
Server.get("/themelist", function(req, res){
	if(!checkAdmin(req, res)) return;
	var ln = req.query.lang || 'ko';
	if(ln == 'ko'){
		res.send({ data: Const.KO_INJEONG, lg: KOEN });
	}else{
		res.send({ data: Const.EN_INJEONG, lg: KOEN });
	}
});
Server.get("/nolist", function(req, res){
	if(!checkAdmin(req, res)) return;
	var ln = req.query.lang || 'ko';
	if(ln == 'ko'){
		res.send({ data: Const.KO_THEME, lg: KOEN, ntd: Const.K_FLAG, ni: Const.K_PUM });
	}else{
		res.send({ data: Const.EN_THEME, lg: KOEN, ntd: Const.K_FLAG, ni: Const.K_PUM });
	}
});
...
Server.get("/gwalli/users", function(req, res){
	...
	if(req.query.name){
		...
		MainDB.users.find([ 'nickname', req.query.name ]).on(function($u){
			if($u) return onSession($u);
			res.sendStatus(404);
		});
	}else{
	...
	function onSession(list){
		...
		Lizard.all(list.map(function(v){
			if(board[v._id]) return null;
			else{
				board[v._id] = true;
				return getProfile(v._id);
			}
		})).then(function(data){
			res.send({ list: data });
		});
...
Server.post("/gwalli/injeong", function(req, res){
	if(!checkAdmin(req, res)) return;
	if(req.body.pw != GLOBAL.PASS && req.body.pw != GLOBAL.WORD) return res.sendStatus(400);
...
Server.post("/gwalli/nodb", onNoDB);
function onKKuTuDB(req, res){
	...
	list.forEach(function(item){
		...
		JLog.log(`${item} is inserted to ${theme} theme.`);
...
function onNoDB(req, res){
	if(!checkAdmin(req, res)) return;
	if(req.body.pw != GLOBAL.PASS && req.body.pw != GLOBAL.WORD) return res.sendStatus(400);
	
	var theme = req.body.theme;
	var flag = req.body.flg;
	var list = req.body.list;
	var pum = req.body.pum || 1;
	var TABLE = MainDB.kkutu[req.body.lang];
	...
	list.forEach(function(item){
		...
		item = item.trim();
		if(!item.length) return;
		JLog.log(`No-INJEONG | WORD: ${item}, THEME: ${theme}, FLAG: ${flag}`);
		TABLE.findOne([ '_id', item ]).on(function($doc){
			if(!$doc) return TABLE.insert([ '_id', item ], [ 'type', pum ], [ 'theme', theme ], [ 'mean', "＂1＂" ], [ 'flag', flag ]).on();
			var means = $doc.mean.split(/＂[0-9]+＂/g).slice(1);
			var len = means.length;
...
Server.post("/gwalli/users", function(req, res){
	if(!checkAdmin(req, res)) return;
	if(req.body.pw != GLOBAL.PASS && req.body.pw != GLOBAL.USERP) return res.sendStatus(400);
	
	var list = JSON.parse(req.body.list).list;
	
	list.forEach(function(item){
		MainDB.users.upsert([ '_id', item._id ]).set(item).on();
		JLog.log(JSON.stringify(item));
	});
	res.sendStatus(200);
});
...
Server.post("/gwalli/write", function(req, res){
	if(!checkAdmin(req, res)) return;
	if(req.body.pw != GLOBAL.PASS) return res.sendStatus(400);
	res.send({url:GLOBAL.WEBHOOK, ok: 200 });
});

};
function noticeAdmin(req, ...args){
	var addr = req.headers['x-forwarded-for'] || req.connection.remoteAddress || req.socket.remoteAddress || req.connection.socket.remoteAddress || req.ip || "::ffff:";
	addr = addr.replace('::ffff:', '');
	var ap = req.ip || "";
	ap = ap.replace('::ffff:', '');
	JLog.info(`[ADMIN] ${req.originalUrl} | ${addr} (${ap}) | ${args.join(' | ')}`);
}
function checkAdmin(req, res){
	if(global.isPublic){
		if(req.session.profile){
			if(GLOBAL.ADMIN.indexOf(req.session.profile.id) == -1 && GLOBAL.WORDUSER.indexOf(req.session.profile.id) == -1 && GLOBAL.MGMTUSER.indexOf(req.session.profile.id) == -1){
