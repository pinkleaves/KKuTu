...
$lib.All.roundReady = function(data){
	...
	playSound('round_start', false, $data.room.opts.faster);
...
$lib.All.turnStart = function(data){
	...
	$data._turnSound = playSound("T"+data.speed, false, $data.room.opts.faster);
...
$lib.All.turnEnd = function(id, data){
	...
	if(data.ok){
		...
		$stage.game.here.hide();
		playSound('timeout', false, $data.room.opts.faster);
	}
	if(data.hint){
