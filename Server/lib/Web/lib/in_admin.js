...
(function(){
	...
	// 끄투 DB에 단어 추가하기
		$.get("/themelist?lang=" + $("#db-lang").val(), function(res){
			var o;
			res.data.forEach(function(v, i){
				o = new Option(res.lg.kkutu["theme_" + v], v);
				//$(o).html(L["theme_" + v]);
				$("#db-theme").append(o);
			});
		});
		$.get("/nolist?lang=" + $("#db-lang").val(), function(res){
			var o;
			res.data.forEach(function(v, i){
				o = new Option(res.lg.kkutu["theme_" + v], v);
				//$(o).html(L["theme_" + v]);
			...
			res.ntd.forEach(function(v, i){
				o = new Option(res.lg.kkutu["flag_" + v], v);
				$("#nodb-flag").append(o);
			});
			res.ni.forEach(function(v, i){
				o = new Option(res.lg.kkutu["class_" + v], v);
				$("#nodb-pum").append(o);
			});
		});
		$("#db-lang").change(function(){
			$("#db-theme").empty();
			$.get("/themelist?lang=" + $("#db-lang").val(), function(res){
				var o;
				o = new Option('주제 선택', '');
				$("#db-theme").append(o);
				$("#db-theme option[value='']").attr('disabled', true);
				res.data.forEach(function(v, i){
					o = new Option(res.lg.kkutu["theme_" + v], v);
					//$(o).html(L["theme_" + v]);
					$("#db-theme").append(o);
			...
			$.get("/nolist?lang=" + $("#db-lang").val(), function(res){
				...
				o = new Option('주제 선택', '');
				$("#nodb-theme").append(o);
				$("#nodb-theme option[value='']").attr('disabled', true);
				res.data.forEach(function(v, i){
					o = new Option(res.lg.kkutu["theme_" + v], v);
					//$(o).html(L["theme_" + v]);
					$("#nodb-theme").append(o);
		...
		$("#db-ok").on('click', function(e){
			...
			var dslb = $("#db-theme").val();
			if(!dslb) return alert('주제를 선택해 주세요.');
			if(forView){
				$("#db-list").val("");
		...
		$("#nodb-ok").on('click', function(e){
			//var forView = $("#nodb-theme").val().charAt() == "~";
			var forView = $("#nodb-search").is(':checked');
			var ds1b = $("#nodb-flag").val();
			var ds2b = $("#nodb-theme").val();
			var ds3b = $("#nodb-pum").val();
			if(!ds1b || !ds2b || !ds3b) return alert('주제, FLAG, 품사 중에 빠진 것이 있습니다.\n확인 후 다시 시도해 주세요.');
			if(forView){
				$("#nodb-list").val("");
				$.get("/gwalli/kkututheme?theme=" + $("#nodb-theme").val()/*.slice(1)*/ + "&lang=" + $("#db-lang").val(), function(res){
					$("#nodb-list").val(res.list.join('\n'));
				});
			}else{
				if($("#db-lang").val() == "en") alert('영어는 검색만 가능합니다.');
				else{
					$.post("/gwalli/nodb", {
						pw: $("#db-password").val(),
						lang: $("#db-lang").val(),
						flg: $("#nodb-flag").val(),
						theme: $("#nodb-theme").val(),
						list: $("#nodb-list").val(),
						pum: $("#nodb-pum").val()
					}, function(res){
						alert(res);
	...
	// 상점 DB 다루기
			...
			$.get("/gwalli/shop/" + $("#shop-word").val(), function(res){
				...
				res.goods.forEach(function(item){
					...
					$r
						...
						.append($("<td>").append(putter("si-" + item._id + "-evtCost", 'y', item.evtCost)))
						.append($("<td>").append(putter("si-" + item._id + "-reqlvl", 'y', item.reqlvl)))
						.append($("<td>").append(putter("si-" + item._id + "-isEvent", 'y', item.isEvent)))
		...
		$("#shop-add").on('click', function(e){
			...
			$("#shop-data").append($("<tr>").attr('id', "gu-" + nid)
				...
				.append($("<td>").append(putter("si-" + nid + "-evtCost", 'y', 0)))
				.append($("<td>").append(putter("si-" + nid + "-reqlvl", 'y', 0)))
				.append($("<td>").append(putter("si-" + nid + "-isEvent", 'y', 0)))
			...
			$("#shop-data tr:visible").each(function(i, o){
				...
				list.push({
					...
					core: {
						...
						evtCost: $data.get(2).value || 0,
						reqlvl: $data.get(3).value || 0,
						isEvent: $data.get(4).value || 0,
						hit: $data.get(5).value || 0,
						term: $data.get(6).value || 0,
						group: $data.get(7).value,
						updatedAt: $data.get(8).value || (new Date()).getTime(),
						options: $data.get(9).value,
					},
					text: {
						name_ko_KR: $data.get(10).value,
						desc_ko_KR: $data.get(11).value,
						name_en_US: $data.get(12).value,
						desc_en_US: $data.get(13).value,
					}
				});
			});
			$.post("/gwalli/shop", {
			...
			$.get("/gwalli/users?id=" + $("#user-id").val() + "&name=" + $("#user-nick").val(), function(res){
				...
				res.list.forEach(function(item){
					...
					$r
						...
						.append($("<td>").append(putter("ud-" + item._id + "-lastaccess", 'g', item.lastaccess)))
						.append($("<td>").append(putter("ud-" + item._id + "-blackt", 't', item.blackt)))
						.append($("<td>").append(putter("ud-" + item._id + "-blackt", 'l', JSON.stringify(item.enhance || {}))));
				});
			});
		});
		$("#user-apply").on('click', function(e){
			...
			$("#user-data tr:visible").each(function(i, o){
				...
				list.push({
					...
					nickname: $data.get(10).value,
					lastaccess: $data.get(11).value,
					blackt: $data.get(12).value,
					enhance: $data.get(13).value
				});
			});
			$.post("/gwalli/users", {
				list: JSON.stringify({ list: list }),
