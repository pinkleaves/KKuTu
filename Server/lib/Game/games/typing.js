...
exports.getTitle = function(){
	...
	var my = this;
	var i, j;
	my.game.theme = my.opts.injpick[Math.floor(Math.random() * my.opts.injpick.length)];
		...
		switch(Const.GAME_TYPE[my.mode]){
			...
			case "KTT":
				getAuto.call(my, my.game.theme).then(function(list){
					...
					else pick(['주제에 단어가 없습니다. 다른 주제를 선택해 주세요.']);
				});
				break;
			default:
	...
	function pick(list, asID){
		...
		for(i=0; i<my.round; i++){
			...
			for(j=0; j<LIST_LENGTH; j++){
				arr.push(asID ? list[Math.floor(Math.random() * len)]._id : list[Math.floor(Math.random() * len)]);
			}
			data.push(arr);
		}
		my.game.lists = data;
...
exports.submit = function(client, text){
	...
	if(my.game.clist[client.game.index] == text){
		...
		if(!my.opts.hack) client.invokeWordPiece(text, 0.5);
	}else{
		client.game.miss++;
