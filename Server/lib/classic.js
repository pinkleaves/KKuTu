...
exports.turnEnd = function(){
	...
	if(target) if(target.game){
		var preChar = getChar.call(my, ptext);
		var preSubChar = getSubChar.call(my, preChar);
		if(my.opts.protect) getAuto.call(my, preChar, preSubChar, 1).then(function(w){
			if(w){
...
exports.submit = function(client, text){
	...
	if(text.indexOf(" ")!=-1 && !my.opts.spacewd){
		if(client.noChat) text = "";
		return client.publish('turnError', { code: 404, value: text }, true);
	}
	if(text.match(/[^A-Za-z\s]/) && !my.opts.specwd && my.rule.lang == 'en'){
		if(client.noChat) text = "";
		return client.publish('turnError', { code: 404, value: text }, true);
	}
	...
	function onDB($doc){
		...
		function preApproved(){
			...
			if(!firstMove && my.opts.nostack){
				getAuto.call(my, preChar, preSubChar, 3, my.game.chain && my.game.chain.length ? (my.game.chain.length + 1) : 1).then(function(w){
					var k, v = false;
					if(!w || !w.length){
						my.game.loading = false;
						...
						for(k in w){
							if(!my.game.chain.includes(w[k]._id)){
								v = true;
								break;
							}
						}
						...
						if(v) approved();
							...
							client.publish('turnError', { code: 415, value: text }, true);
							if(client.robot && !client.plient) my.readyRobot(client);
						}
					}
				});
	...
	var kdas = my.opts.spacewd || my.opts.specwd;
	if(my.opts.dongsa && l == "ko"){
...
/*exports.getScore = function(text, delay, ignoreMission){
	...
	score = Const.getPreScore(text, my.game.chain, tr, undefined, undefined, undefined, undefined, true);
	
	if(my.game.dic[text]) score *= 15 / (my.game.dic[text] + 15);
	if(!ignoreMission) if(arr = text.match(new RegExp(my.game.mission, "g"))){
		score += score * 0.45 * arr.length;
...
exports.getScore = function(text, delay, ignoreMission){
	...
	score = Const.getPreScore2(text, my.game.chain, tr);
...
exports.readyRobot = function(robot){
	...
	var w, text, i;
	var lmax;
	var mymd = Const.GAME_TYPE[my.mode];
	if(mymd == "EAP" || mymd == "KAP") var isRev = true;
		...
		if(list.length){
			...
			if(ROBOT_HIT_LIMIT[level] > list[0].hit) denied();
				...
				if(level >= 3 && !robot._done.length){
					if(Math.random() <= 0.4 || level >= 4) list.sort(function(a, b){ return b._id.length - a._id.length; });
					pickList(list);
				}else pickList(list);
			}
		}else denied();
	...
	function pickList(list){
		...
		if(w){
			...
			if(my.game && my.game.chain && my.game.chain.indexOf(text) != -1 && !my.game.late){
				pickList(list);
			} else {
				after();
			}
	...
	function after(){
		//JLog.log("Bot_Type: Msg #AiLevel" + String(level) + ": " + text + " on Classic mode");
...
function getAuto(char, subc, type, ln){
	/* type
		0 무작위 단어 하나
		1 존재 여부
		2 단어 목록
	*/
	...
	var adv, adc;
	var gty = gameType == "KGT" ? "KSH" : gameType;
	var key = gty + "_" + keyByOptions(my.opts);
	...
	function produce(){
		...
		if(type == 3){
			DB.kkutu[my.rule.lang].find.apply(this, aqs).limit(ln ? ln : 1000000).on(function($md){
				//forManner($md);
				/*if(my.game.chain) aft($md.filter(function(item){ return !my.game.chain.includes(item); }));
				else */
				aft($md);
			});
		}else DB.kkutu[my.rule.lang].find.apply(this, aqs).limit(bool ? 1 : 123).on(function($md){
			forManner($md);
			if(my.game.chain) aft($md.filter(function(item){ return !my.game.chain.includes(item); }));
			else aft($md);
		});
...
function getChar(text){
	var my = this;
	var eql = Math.round(Math.random());
	eql = (eql == 0) ? -0.5 : 0.5;
	switch(Const.GAME_TYPE[my.mode]){
		case 'EKT': return text.slice(text.length - 3);
		case 'KMT': return text.slice(text.length - 3);
		case 'ESH':
		case 'KKT':
		case 'EKD':
		case 'KSH': return text.slice(-1);
		case 'KAP': return text.charAt(0);
		case 'EAP': return text.charAt(0);
		case 'KGT': return (text.length % 2) ? text.charAt((text.length - 1) / 2) : text.charAt(((text.length - 1) / 2) + eql);
	}
};
function getSubChar(char){
	...
	switch(Const.GAME_TYPE[my.mode]){
		...
		case "KMT":
			if(char.length > 2) r = char.slice(1);
			break;
		case "KKT": case "KSH": case "KAP": case "KGT":
			k = c - 0xAC00;
			...
			break;
		case "ESH": case "EKD": case "EAP": default:
			break;
	}
	return r;
}