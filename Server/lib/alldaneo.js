		...
		function onDB($doc){
			...
			if($doc){
				switch(Const.GAME_TYPE[my.mode]){
					case 'KSD':
					case 'ESD':
						if($doc.theme.match(toRegex(my.opts.injpick.join('|'))) == null) denied(407);
						else preApproved();
						break;
					default:
						preApproved();
...
exports.readyRobot = function(robot){
	...
	if(my.rule.lang == 'ko'){
		pl = 'ko';
		var rtheme = Const.KO_INJEONG;
		rtheme = rtheme.concat(Const.KO_THEME);
		var rand = Math.floor(Math.random() * rtheme.length); //0~주제개수
		var nwd = rtheme[rand];
	}else{
		pl = 'en';
		var rtheme = Const.EN_INJEONG;
		rtheme = rtheme.concat(Const.EN_THEME);
		var rand = Math.floor(Math.random() * rtheme.length);
		var nwd = rtheme[rand];
	}
	if(well){	
		getAuto.call(my, nwd, 2).then(function(list){
			if(list.length){
